...
const CATALOGO = [
  ...
  { n: 'São Paulo', s: 'SAO', uf: 'SP', c: ['#E4002B', '#161616'], ser: 'A', l: 'https://upload.wikimedia.org/wikipedia/commons/f/f4/S%C3%A3o_Paulo_Futebol_Clube_logo_%282022%29.svg' },
  ...
  { n: 'Atlético-MG', s: 'CAM', uf: 'MG', c: ['#161616', '#FFFFFF'], ser: 'A', l: 'https://upload.wikimedia.org/wikipedia/commons/4/41/Logo_of_Clube_Atl%C3%A9tico_Mineiro.svg' },
  ...
  { n: 'Grêmio', s: 'GRE', uf: 'RS', c: ['#0D80BF', '#161616'], ser: 'A', l: 'https://upload.wikimedia.org/wikipedia/commons/0/08/Gremio_logo.svg' },
  ...
  { n: 'Vitória', s: 'VIT', uf: 'BA', c: ['#E4002B', '#161616'], ser: 'A', l: 'https://upload.wikimedia.org/wikipedia/commons/1/15/Esporte_Clube_Vit%C3%B3ria_%282024%29.svg' },
  ...
  { n: 'América-MG', s: 'AME', uf: 'MG', c: ['#0A7A33', '#FFFFFF'], ser: 'B', l: 'https://upload.wikimedia.org/wikipedia/commons/6/6f/Escudo_oficial_do_Am%C3%A9rica_Futebol_Clube.svg' },
...
const FUNDOS = { grid: 'Grade', dots: 'Pontos', aurora: 'Aurora', bandeiras: 'Bandeirinhas', none: 'Liso' };
// varal de bandeirinhas (decoração, não escudo) nas cores dos times da própria copa — uma fileira fina no topo, estilo festa
function fundoBandeirasSvg(times, corLinha) {
  const paleta = (times || []).map(t => [safeColor(t.c && t.c[0], '#888'), safeColor(t.c && t.c[1], '#ccc')]);
  const cores = paleta.length ? paleta : [['#d4a847', '#0d0d12'], ['#38bdf8', '#0d0d12'], ['#22c55e', '#0d0d12'], ['#ef4444', '#0d0d12'], ['#c084fc', '#0d0d12'], ['#f0c75a', '#0d0d12']];
  const w = 46, h = 40, totalW = cores.length * w;
  const pecas = cores.map((c, i) => {
    const x = i * w + w / 2, dy = i % 2 === 0 ? 0 : 5, rot = i % 2 === 0 ? -4 : 4;
    return `<g transform="translate(${x},${7 + dy}) rotate(${rot})">
      <path d="M-11,0 L11,0 L0,20 Z" fill="${c[0]}"/><path d="M0,0 L11,0 L0,20 Z" fill="${c[1]}"/>
      <path d="M-11,0 L11,0 L0,20 Z" fill="none" stroke="#000" stroke-opacity=".18" stroke-width="1"/>
    </g>`;
  }).join('');
  return `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 ${totalW} ${h}" width="${totalW}" height="${h}">
    <line x1="0" y1="5" x2="${totalW}" y2="5" stroke="${corLinha}" stroke-width="1.5" stroke-opacity=".45"/>${pecas}</svg>`;
}
let fundoBandeirasCache = { key: '', url: '' };
function fundoBandeirasUrl(times, corLinha) {
  const key = corLinha + '|' + (times || []).map(t => t.c && t.c.join(',')).join(';');
  if (fundoBandeirasCache.key !== key) fundoBandeirasCache = { key, url: 'data:image/svg+xml,' + encodeURIComponent(fundoBandeirasSvg(times, corLinha)) };
  return fundoBandeirasCache.url;
}
const FONTES = {
...
function aplicarTema(tema, els, times) {
  ...
  els.filter(Boolean).forEach(el => {
    ...
    const bgfx = el.querySelector && el.querySelector('.cx-bgfx');
    if (bgfx) {
      if (T.fundo === 'bandeiras') { bgfx.style.backgroundImage = `url("${fundoBandeirasUrl(times, c.accent)}")`; bgfx.style.backgroundSize = 'auto 76px'; bgfx.style.backgroundPosition = 'top center'; bgfx.style.backgroundRepeat = 'repeat-x'; }
      else { bgfx.style.backgroundImage = ''; bgfx.style.backgroundSize = ''; bgfx.style.backgroundPosition = ''; bgfx.style.backgroundRepeat = ''; }
    }
...
function aplicarTemaAtual() {
  const T = aplicarTema(S.E ? S.E.tema : TEMA_PADRAO, [S.raiz, S.camada], S.E ? S.E.times : []);
  return T;
}
...
function htmlApresentBtn(ativo) {
  return `<button class="cx-chip ${ativo ? 'on' : ''}" style="margin-left:auto" data-cx="apresentToggle" aria-pressed="${ativo}">${ico(ativo ? 'pause' : 'play')} ${ativo ? 'Sair da apresentação' : 'Apresentação'}</button>`;
}
function vGrupos() {
  const E = S.E;
  if (!E.grupos || !E.grupos.length) return vazioSemEstrutura();
  const pontos = E.cfg.formato === 'pontos';
  const multi = !pontos && E.grupos.length > 1;
  const info = pontos ? '' : `<div class="cx-sec">${E.grupos.length} grupo${E.grupos.length > 1 ? 's' : ''} · classificam ${E.cfg.classificam} por grupo${multi ? htmlApresentBtn(S.apresent) : ''}</div>`;
  ...
  if (multi && S.apresent) {
    const i = clamp(S.apresentI, 0, E.grupos.length - 1);
    const dots = E.grupos.map((g, gi) => `<button class="cx-adot ${gi === i ? 'on' : ''}" data-cx="apresentDot" data-i="${gi}" aria-label="Ir para o grupo ${esc(g.id)}"></button>`).join('');
    return `${info}<div class="cx-ggrid cx-apresent" style="grid-template-columns:minmax(0,520px)"><div data-cx="apresentProx" role="button" tabindex="0" aria-label="Próximo grupo" title="Toque para passar para o próximo grupo">${htmlTabelaGrupo(E.grupos[i], i)}</div></div><div class="cx-adots">${dots}</div><p class="cx-hint" style="margin-top:10px">${ico('play')} Passando os grupos sozinho — toque no card ou nas bolinhas pra pular.</p>`;
  }
...
function htmlPicker() {
  ...
  if (p.modo === 'br') {
    corpo = `${seg}<div class="cx-row" style="align-items:flex-end"><div class="cx-f" style="margin:0;flex:2"><label for="cx-busca">Buscar</label><input class="cx-in" id="cx-busca" data-cxi="pickBusca" placeholder="Nome ou estado (ex.: SP)" value="${esc(p.busca)}" autocomplete="off"></div><div class="cx-chips" style="margin:0;flex:none">${[['todos', 'Todos'], ['A', 'Série A'], ['B', 'Série B']].map(([k, n]) => `<button class="cx-chip ${p.serie === k ? 'on' : ''}" data-cx="pickSerie" data-s="${k}">${n}</button>`).join('')}</div></div>
      ...
      <div class="cx-cat" id="cx-cat"></div><p class="cx-hint" style="margin-top:14px">Times do Brasileirão já vêm com o brasão oficial. Dá para trocar a imagem, cor e sigla depois de adicionar.</p>`;
...
/* ---------- apresentação: passa os grupos sozinho (clicar também avança) ---------- */
let apresentTimer = null;
function apresentarAgendar() {
  clearTimeout(apresentTimer);
  apresentTimer = setTimeout(() => {
    if (!S.apresent || !S.E || !S.E.grupos || !S.E.grupos.length) return;
    S.apresentI = (S.apresentI + 1) % S.E.grupos.length; S.animar = true; render(); apresentarAgendar();
  }, 6500);
}
function apresentarLigar() { S.apresent = true; S.apresentI = 0; S.animar = true; render(); apresentarAgendar(); }
function apresentarDesligar() { S.apresent = false; clearTimeout(apresentTimer); apresentTimer = null; }
function apresentarProx() {
  if (!S.E || !S.E.grupos || !S.E.grupos.length) return;
  S.apresentI = (S.apresentI + 1) % S.E.grupos.length; S.animar = true; render(); apresentarAgendar();
}

const ACOES = {
  aba(el) { if (S.apresent && el.dataset.a !== 'grupos') apresentarDesligar(); S.aba = el.dataset.a; S.animar = true; render(); },
  apresentToggle() { if (S.apresent) apresentarDesligar(); else apresentarLigar(); render(); },
  apresentProx() { apresentarProx(); },
  apresentDot(el) { const i = +el.dataset.i; if (!Number.isFinite(i)) return; S.apresentI = i; S.animar = true; render(); apresentarAgendar(); },
  jogo(el) {
...
/* ---------- não deixar a tela dormir (útil pra deixar rodando numa TV) ----------
   tenta a Wake Lock API primeiro; onde não existir (navegador de TV mais antigo), cai pro truque
   clássico de um vídeo "vivo" tocando escondido — aqui gerado via canvas.captureStream(), sem
   depender de nenhum arquivo de vídeo embutido */
let wakeLock = null, wakeVideo = null, wakeTimer = null;
async function telaLigadaPedir() {
  try {
    if (navigator.wakeLock) { wakeLock = await navigator.wakeLock.request('screen'); return; }
  ...
  try {
    ...
    let t = 0;
    const desenhar = () => { t = 1 - t; ctx.fillStyle = t ? '#000000' : '#010101'; ctx.fillRect(0, 0, 2, 2); };
    desenhar();
    ...
    await v.play().catch(() => {});
    wakeVideo = v; wakeTimer = setInterval(desenhar, 1000);
  } catch (e) { /* nada a fazer, segue sem trava */ }
}
function telaLigadaSoltar() {
  try { wakeLock && wakeLock.release(); } catch (e) { /* já solta */ }
  wakeLock = null;
  clearInterval(wakeTimer); wakeTimer = null;
  if (wakeVideo) {
    try { const st = wakeVideo.srcObject; if (st) st.getTracks().forEach(tr => tr.stop()); wakeVideo.pause(); wakeVideo.remove(); } catch (e) { /* ignora */ }
    wakeVideo = null;
  }
}
document.addEventListener('visibilitychange', () => { if (document.visibilityState === 'visible' && S.raiz && document.body.contains(S.raiz)) telaLigadaPedir(); });
